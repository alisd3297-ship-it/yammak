    ...
    queryFn: async () => {
      ...
      return {
        ...
        notifications: {
          ...
          byKind: [
            ...notifications
              .reduce((map, n) => map.set(n.kind, (map.get(n.kind) ?? 0) + 1), new Map<string, number>())
              .entries(),
          ].sort((a, b) => b[1] - a[1]),
        },
        errors: {
                  ...
                  {data.orders.byType.map(([type, row]) => (
                    <li key={type} className="flex items-center justify-between gap-3 text-xs">
                      ...
                      <span className="text-muted-foreground">
                        الإجمالي {row.total} · ناجحة {row.success} · ملغاة {row.failed} · نسبة النجاح{" "}
                        {pct(row.success, row.total)}
                      </span>
                    </li>
                  ))}
                </ul>
                  ...
                  {data.errors.groups.map((g) => (
                    <li key={`${g.kind}-${g.message}`} className="rounded-xl bg-muted/50 p-3">
                      <div className="flex items-center justify-between gap-2 text-[11px] text-muted-foreground">
                        ...
                        <span>تكرار {g.count} · آخر مرة {fmtDate(g.last)}</span>
                      </div>
