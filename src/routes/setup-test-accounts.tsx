  ...
  head: () => ({
    meta: [
      { title: "تجهيز حسابات الاختبار | لبابك" },
      {
        name: "description",
        content:
          "مسار محمي برمز سري لتجهيز حسابات اختبار: زبون، مندوب توصيل، وتاجر، بالأدوار الفعلية للمشروع.",
      },
      { property: "og:title", content: "تجهيز حسابات الاختبار | لبابك" },
      {
        property: "og:description",
        content: "إنشاء أو تحديث كلمات مرور حسابات الاختبار برمز إعداد سري.",
      },
...
const KINDS = [
  ...
  {
    kind: "driver",
    email: "qa.driver@lubabak.test",
    label: "مندوب توصيل اختبار",
    role: "worker + delivery",
  },
  {
    kind: "vendor",
    email: "qa.vendor@lubabak.test",
    label: "تاجر اختبار",
    role: "provider + متجر",
  },
  {
      ...
      if (!res.ok) {
        const messages: Record<string, string> = {
          ...
          server_token_missing:
            "رمز الإعداد غير مضبوط على الخادم — أضِف ADMIN_SETUP_TOKEN ثم أعد النشر",
  ...
  return (
    <div className="app-shell px-5 py-10">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        محمي برمز الإعداد السري المخزّن في إعدادات المشروع. الأدوار المستخدمة هي الأدوار الموجودة
        فعلاً: زبون <span dir="ltr">customer</span>، مندوب <span dir="ltr">worker</span> بنوع{" "}
        <span dir="ltr">delivery</span>، وتاجر <span dir="ltr">provider</span>. لا صلاحيات إدارية
        لأي منها.
      </p>
