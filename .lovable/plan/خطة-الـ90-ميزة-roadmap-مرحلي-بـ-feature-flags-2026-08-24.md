...
## إضافات لاحقة على قائمة العمل (تخطيط فقط — غير منفّذة)
- **91. مكينة نقل**: خدمة نقل بمركبة حمل صغيرة (مكينة) — تُبنى كنوع مركبة/خدمة إضافي فوق «التوصيل الخاص» و`vehicle_type` الحالي، بتسعير مستقل في `fee_rules` وخلف feature flag، بدون تعديل دورة الطلب.
- **92. ستوتة نقل**: خدمة نقل بستوتة (تكتك ثلاثي) — نفس الأسلوب: نوع مركبة إضافي + تسعير مستقل + شرط توزيع في `runDispatch` مقيّد بنوع المركبة، خلف feature flag.
مكانها المقترح في التسلسل: مع المرحلة 4 (العمليات واللوجستيك) أو بعدها، لاعتمادها على التوزيع ونطاقات المندوبين.
